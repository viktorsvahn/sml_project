...
# (i) Does the distance to the nearest horn affect whether a person hears the siren or
# not?
...
def answer_distance(df):
    fig = px.histogram(
        ...
        barmode="overlay",
        title="Normalised histograms of distance for persons<br>who heard or did not \
            hear the siren",
    )
    fig.update_layout(
    ...
    fig.show()
    print(
        "(i) Does the distance to the nearest horn affect whether a person hears the \
            siren or not?"
    )
    print(
        "There is a clear trend towards lower fraction of people hearing the siren for \
            larger distances up to around 20000 m (~e^10)"
    )
    print(
        "There is a cluster of data points above 20000 m. Considering the previous trend \
        that cluster have a surprisinlgy high fraction of people that heard the siren."
    )
    print(
        "It is also interesting to see that several people at distances between 50 and \
    110 km from the siren claim to have heard it. These are likely to be errors \
    in the data?"
    )
...
def answer_age(df):
    fig = px.histogram(
        ...
        histnorm="probability",
        barmode="overlay",
        title="Normalised histograms of age for persons<br>who heard or did not hear the \
            siren",
    )
    fig.update_layout(
        legend=dict(orientation="h", yanchor="top", y=-0.1, xanchor="left", x=0)
    ...
    fig.show()
    print(
        "(ii) Are the people who hear the siren younger than the people who do not hear \
            it?"
    )
    print(
        "There is a clear tendency towards lower fraction of older people hearing the \
            siren"
    )
...
def answer_direction(df):
    ...
    fig = px.bar(
        ...
        title="Fraction of people that heard the siren,<br>grouped by their angle \
            towards the closest siren",
    )
    fig.update_layout(
        legend=dict(orientation="h", yanchor="top", y=-0.1, xanchor="left", x=0)
    )
    fig.show()
    print(
        "(iii) Does the direction towards the nearest horn affect whether a person \
            hears the siren or not?"
    )
    print(
        "While it might be plausible that the direction towards the closest siren, \
            combined with eg the wind direction, have an impact on the chance of \
            hearing the siren, this cannot be observed in the data."
    )
